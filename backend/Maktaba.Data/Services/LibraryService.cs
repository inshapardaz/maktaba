...
public class LibraryService : ILibraryService, ILibraryPathProvider
{
    ...
    // column, and issue #30's Periodical.Language column) - a cheap, representative stand-in for
    // "is this database current" without needing full EF Core migrations, which this project
    // deliberately doesn't use. Every future schema-breaking change needs its own probe added
    // here, or an upgrading user's existing metadata.db won't be recognized as stale and requests
    // against the new column/table will throw instead of transparently rebuilding.
    private static async Task<bool> IsCurrentSchemaAsync(MaktabaDbContext db, CancellationToken ct)
    {
        try
        {
            ...
            await db.Periodicals.Select(p => p.Language).Take(1).ToListAsync(ct);
            return true;
        }
